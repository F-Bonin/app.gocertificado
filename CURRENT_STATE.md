...
* [x] **Segurança e UX (Páginas Públicas):** As views `RegistrationCreateView` (Solicitação de Certificado) e `EventRegistrationCreateView` (Inscrição em Evento) foram refatoradas para utilizar o template de interrupção visual com cronômetro (`time_locked.html`) em vez de respostas HTTP HTML estáticas para bloqueios de período.
