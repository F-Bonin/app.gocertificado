...
* [x] **Arquitetura de Eventos Recorrentes:** Implementada a base de dados para "Eventos Recorrentes" (`RecurringEvent`) e "Encontros" (`EventSession`), isolada dos eventos pontuais. Adicionado o campo `event_type` para classificar eventos como "Programado" (com certificado) ou "Personalizado" (apenas lista de presença). Implementada trava arquitetural via override do método `save` para forçar `no_certificate = True` in eventos personalizados. Implementada a camada de Controle com `RecurringEventForm` e `EventSessionFormSet` (Inline Formset) e as Views de CRUD com proteção multitenant e validação transacional. O fluxo Mestre-Detalhe foi blindado contra "Falhas Silenciosas" através do override dos métodos `post`, `form_valid` e `form_invalid` nas Views, garantindo a notificação explícita de erros via Django Messages e a integridade da carga horária (`hours`) no formulário mesmo em cenários de inatividade do frontend. O campo auxiliar `num_signatures` no `RecurringEventForm` foi definido como `required=False`, eliminando bloqueios de salvamento quando o campo não é renderizado no template, permitindo o salvamento com sucesso dos Eventos Recorrentes. A interface de Evento Recorrente foi integrada nativamente ao `management_form` do Django, convertida para submissão via POST puro (Submit) e recebeu um motor de feedback visual do Bootstrap (`was-validated`) para erros globais e específicos de sessões. Implementada a funcionalidade de "Raio-X" de erros no template, que expõe detalhadamente as falhas de validação do Backend (form e formset), listando os motivos exatos de rejeição para cada encontro. O Backend foi blindado contra campos vazios gerados por inatividade do Frontend através de uma trava de segurança no método `clean()` de `EventSessionForm`, garantindo que a carga horária (`hours`) receba zero caso venha nula. Foi aplicada a formatação ISO 8601 (`%Y-%m-%d` e `%H:%M`) nos widgets de data e hora do `EventSessionForm`, resolvendo o bug de renderização de valores no modo de edição dos inputs HTML5. O modelo `EventSession` foi expandido para suportar segmentação detalhada de endereços (Condomínio/Casa) com campos específicos para bloco, andar, apartamento e país. Implementada obrigatoriedade estrita nos campos core e validação condicional inteligente de endereço (Condomínio) no backend via `clean()` em `EventSessionForm`, com injeção automática de asteriscos (*) de sinalização visual nos rótulos. A obrigatoriedade do campo Complemento foi removida da classe `EventSessionForm` para garantir flexibilidade em endereços simplificados. Implementada lógica reativa Vanilla JS com *Event Delegation* para gerenciar a exibição condicional da Chave de Acesso (Senha) e a alternância de campos de endereço. A responsabilidade da Chave de Acesso foi centralizada no Evento Pai através do novo campo `global_passkey`, removendo a necessidade de senhas individuais por encontro (`session_passkey`). A listagem de Eventos Recorrentes (`recurring_event_list.html`) recebeu a aplicação do padrão *Progressive Disclosure*, com um Mini-Dashboard integrado para gestão de links públicos (Inscrição/Certificado) e controle administrativo de sessões e presença, preparando a interface para as fases de credenciamento. [20/04/2026]
* [x] **Refatoração Global de Rótulos e Higienização:** Centralizada a "Fonte da Verdade" dos rótulos diretamente nos modelos. O campo `institution_name` foi renomeado para "Nome do Local" e `custom_reg_form`/`custom_cert_form` foram padronizados como "Formulário de Inscrição" e "Formulário de Solicitação de Certificado" em todos os modelos (`Course`, `RecurringEvent`, `EventSession`, `Registration`). Realizada a higienização completa dos templates administrativos e públicos (`course_form.html`, `recurring_event_form.html`, `form.html`, `event_form.html`, etc.), eliminando strings hardcoded e comentários obsoletos. O sistema agora apresenta uma interface coesa e profissional, livre de referências inconsistentes a "Instituição". [20/04/2026]
* [x] **Refatoração Global de Rótulos (Models):** Centralizada a "Fonte da Verdade" dos rótulos diretamente nos modelos. O campo `institution_name` foi renomeado para "Nome do Local" e `custom_reg_form` para "Formulário de Inscrição" nos modelos (`Course`, `RecurringEvent`, `EventSession`, `Registration`) via `verbose_name`. O campo `hours` no modelo `EventSession` foi renomeado para "Carga horária do encontro" para garantir consistência com a nomenclatura de encontros. Os blocos redundantes de `labels` foram removidos dos formulários `CourseForm` e `RecurringEventForm`. [20/04/2026]
* [x] **Refatoração de Formulários (Recorrentes):** Refatorados os formulários `RecurringEventForm` e `EventSessionForm` para incluir campos de carga horária em modo `readonly`, automação de rótulos (Início/Término do Evento) e suporte a cálculos automáticos de workload. O `EventSessionForm` foi expandido para expor os novos campos de segmentação de endereço (`location_type`, `condominium_name`, `address_block`, etc.) com suporte a widgets estilizados. [20/04/2026]
...
* [x] **Bugfix de Redirecionamento (Trava de Duplicidade):** Corrigido o erro `'NoneType' object has no attribute 'dict'` al interceptar solicitações duplicadas, garantindo a injeção do `self.object` antes da chamada de `get_success_url()` [25].
...
* [x] **Segurança Anti-Fraude:** Implementada trava de segurança na View de solicitação de certificado que impede o rotu de identidade e a sobrescrita de dados sensíveis (Nome, CPF, RG, Data de Nascimento). O sistema agora valida o nome informado contra o nome cadastrado na inscrição (com normalização de strings) e blinda os campos core da identidade.
...
* [x] **Correções e Melhorias Técnicas:** O método `get_context_data` foi restaurado nas Views de inscription/solicitação para corrigir a exibição dinâmica dos dados do evento (Nome, Data, Carga Horária, Local) no frontend. [50]
